    ...
    <div className={`h-screen lg:p-[0.8rem] flex flex-col select-none font-circular`}>
      <div className='lg:hidden'>
        <div className=' bg-DeepNightBlack text-LightGray w-full h-10 flex items-center justify-between px-2 lg:hidden relative'>
          <div className='icon flex items-center gap-x-2' onClick={(e) => setIntro(!intro)}>
            <span className='icon border-2 text-[#a3ff04] border-[#a3ff04] p-1 text-sm rounded-lg'>
              <SlOptionsVertical />
            </span>
            <div className='text-[#556a2e] absolute -right-1 -bottom-1'>
              <FaMousePointer />
            </div>
          </div>
      ...
      <div className='flex relative h-full justify-between gap-x-3'>
        ...
        <div className={`hidden lg:block absolute lg:w-20 lg:relative bg-DeepNightBlack shadow-2xl rounded-xl overflow-hidden`}>
          <div onClick={(e) => setIsOpen(!isOpen)} className='bg-MidNightBlack text-[#a3ff04] hidden lg:flex items-center h-16 justify-center text-2xl '>
            <span className='icon border-2 border-[#a3ff04] p-2 rounded-xl'>
              {' '}
              <FaBars />
            </span>
          </div>
          <span className='absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 rotate-90 flex items-center justify-center text-center text-xl text-[#8cff004a] font-extrabold tracking-widest'>
            .rzoktan
          </span>
        </div>
